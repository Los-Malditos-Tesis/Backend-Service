...
import { obfuscateApiKey, obfuscatePass, obfuscateToken } from "../utils/obfuscate/obfucates.js";
...
export const loginCameraController = async (req, res, next) => {
    try {
        Log.infoCtx(req.ctx, authController + consoleKeys.StartKey, consoleKeys.RequestKey, obfuscateApiKey(req.body))
        const serviceResp = await authCamera(req.body, req.ctx)

        Log.infoCtx(req.ctx, authController + consoleKeys.SuccessKey, consoleKeys.ResponseKey, obfuscateToken(serviceResp))

        return generalResponse(res, 200, CODES.SUCCESS.OK, 'Camera logged in successfully', serviceResp);
    } catch (e) {
        Log.errorCtx(req.ctx, authController + consoleKeys.FailKey, e);
        next(e);
    } finally {
