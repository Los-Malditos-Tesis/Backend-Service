...
import {
  ...
  getStructureWithCameras,
} from "../service/warehouse_service.js";
...
export const createWarehouseController = async (req, res, next) => {
  try {
    Log.infoCtx(
      req.ctx,
      warehouseController + consoleKeys.StartKey,
    ...
    Log.infoCtx(
      req.ctx,
      warehouseController + consoleKeys.SuccessKey,
...
export const getWarehouseInventoryController = async (req, res, next) => {
  try {
    ...
    const response = await getInventoryByLocation(
      req.params.id,
    ...
    return generalResponse(
      ...
      200,
      CODES.SUCCESS.OK,
      "Warehouse inventory get successfully",
      response,
    );
  } catch (e) {
    Log.errorCtx(req.ctx, warehouseController + consoleKeys.FailKey, e);
...
export const getWarehouseStructureController = async (req, res, next) => {
  try {
    ...
    return generalResponse(
      ...
      200,
      CODES.SUCCESS.OK,
      "Warehouse structure get successfully",
      response,
    );
  } catch (e) {
    Log.errorCtx(req.ctx, warehouseController + consoleKeys.FailKey, e);
