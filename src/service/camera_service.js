...
import {
  ...
  updateCamera,
  deleteById,
} from "../repositories/camera_repository.js";
...
export const registerCamera = serviceHandler(
  ...
  async (data = {}, ctx) => {
    ...
    if (existsCamera)
      throw new AppError(
        "El codigo de camara esta en uso",
        400,
        CODES.CAMERA.ALREADY_EXISTS,
      );

    data.api_key = crypto.randomBytes(32).toString("hex");

    await findLocationById(data.location_id, ctx);
    Log.infoCtx(
      ctx,
      cameraService + consoleKeys.InformationKey,
      consoleKeys.InformationKey,
      obfuscateApiKey(data),
    );
