...
import {
  ...
  deleteById,
  search,
  getWarehouseInventoryByLocation,
  getWarehouseStructure,
  findLocationByQrInWarehouse,
  update,
} from "../repositories/warehouse_repository.js";
...
export const createWarehouse = serviceHandler(
  ...
  async (warehouseData = {}, ctx) => {
    Log.infoCtx(
      ctx,
      warehouseService + consoleKeys.StartKey,
    ...
    Log.infoCtx(
      ctx,
      warehouseService + consoleKeys.SuccessKey,
  ...
  async (warehouseData = {}, ctx) => {
    ...
    if (existWarehouse.user_id !== warehouseData.user_id) {
      ...
      if (existUser.roles.some((role) => role.name === "USER"))
        throw new AppError(
          "El usuario no tiene permiso para ser el administrador de un almacen",
          403,
          CODES.WAREHOUSE.NOT_PERMISSIONS,
        );
    }
...
export const deleteWarehouse = serviceHandler(
  ...
  async (id, ctx) => {
    ...
    if (warehouseWithInventory.Locations.length > 0)
      throw new AppError(
        "El almacen no se puede eliminar porque tiene ubicaciones",
        400,
        CODES.WAREHOUSE.HAS_STOCK,
      );
...
export const getInventoryByLocation = serviceHandler(
  ...
  async (warehouseId, locationId, ctx) => {
    ...
    const inventory = await getWarehouseInventoryByLocation(
      warehouseId,
      locationId,
      ctx,
    );
...
export const findLocationByQr = serviceHandler(
  ...
  async (warehouseId, zoneQr, ctx) => {
    ...
    const location = await findLocationByQrInWarehouse(
      warehouseId,
      zoneQr,
      ctx,
    );
    ...
    if (!location || location.Locations.length === 0)
      throw new AppError(
        "La zona escaneada no pertenece a este almacén o no existe",
        404,
        CODES.WAREHOUSE.NOT_FOUND,
      );
