...
export const getInventoryByLocation = serviceHandler(
    ...
    async (warehouseId, locationId, ctx) => {
        ...
        if (!location)
            throw new AppError('La ubicación no existe', 404, warehouseCodes.NOT_FOUND);
...
export const getStructureWithCameras = serviceHandler(
    ...
    async (warehouseId, ctx) => {
        ...
        if (!structure)
            throw new AppError('Almacén no encontrado', 404, warehouseCodes.NOT_FOUND);
...
export const findLocationByQr = serviceHandler(
    ...
    async (warehouseId, zoneQr, ctx) => {
        ...
        if (!location || location.Locations.length === 0)
            throw new AppError('La zona escaneada no pertenece a este almacén o no existe', 404, warehouseCodes.NOT_FOUND);
