...
export const authorizeMiddleware = (requiredRoles = []) => {
    const message = 'User does not have the required permissions to access this resource';

    return async (req, res, next) => {
        Log.infoCtx(req.ctx, authorizeMiddlewareKey + consoleKeys.StartKey)
        const roles = req.user.roles.map(role => role.id);

        if (
            !req.user ||
            !requiredRoles.some(role =>roles.includes(role))
        ) {
            Log.warnCtx(req.ctx, authorizeMiddlewareKey + consoleKeys.FailKey, consoleKeys.ErrorKey, message)
